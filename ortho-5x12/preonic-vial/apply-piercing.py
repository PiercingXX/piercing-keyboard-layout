#!/usr/bin/env python3
"""PiercingXX Preonic keymap tool (Drop Preonic rev3, Vial firmware).
...
Usage:
  ...
  ./apply-piercing.py --restore preonic-keymap-backup-2026-07-16.bin
...
def MO(layer): return 0x5220 + layer
...
GRV=0x35,COMM=0x36,DOT=0x37,SLSH=0x38,F1=0x3A,F2=0x3B,F3=0x3C,F4=0x3D,F5=0x3E,
F6=0x3F,F7=0x40,F8=0x41,F9=0x42,F10=0x43,F11=0x44,F12=0x45,PSCR=0x46,INS=0x49,
DEL=0x4C,RGHT=0x4F,LEFT=0x50,DOWN=0x51,UP=0x52,NLCK=0x53,PSLS=0x54,PAST=0x55,
...
L0 = [  # base — standard positions; OS layout renders Piercing
 ...
 [k['TAB'],k['Q'],k['W'],k['E'],k['R'],k['T'],k['Y'],k['U'],k['I'],k['O'],k['P'],k['MPLY']],
 [k['BSPC'],k['A'],k['S'],k['D'],k['F'],k['G'],k['H'],k['J'],k['K'],k['L'],k['SCLN'],k['QUOT']],
 [k['LSFT'],LT(1,k['Z']),LT(2,k['X']),k['C'],k['V'],k['B'],k['N'],k['M'],k['COMM'],k['DOT'],k['SLSH'],k['RSFT']],
 [k['LCTL'],k['LGUI'],k['LALT'],NO,k['RALT'],k['ENT'],k['SPC'],k['SPC'],k['PSCR'],k['VOLD'],k['VOLU'],k['ESC']],
]
...
L1 = [  # hold the ,-key (tap = comma): F-keys, vim arrows, symbols
...
L2 = [  # hold the c-key (tap = c): numpad (Piercing-safe digits)
 [TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,TRNS,k['NLCK'],k['PSLS'],k['PAST'],k['PMNS'],k['INS']],
...
L3 = [[TRNS]*12 for _ in range(5)]  # cleared (was unreachable leftovers)

def to_matrix(vis):
    """visual 5x12 -> Preonic rev3 electrical matrix (10x6)."""
    m = []
    for vr in range(4): m.append(vis[vr][0:6])
    for vr in range(4): m.append(vis[vr][6:12])
    b = vis[4]
    m.append(b[0:3] + b[9:12])   # row 8: c0-c2, c9-c11
    m.append(b[6:9] + b[3:6])    # row 9: c6-c8, c3-c5
    return m

def build_buffer():
    out = b""
    for layer in (L0, L1, L2, L3):
        for row in to_matrix(layer):
            out += struct.pack(">6H", *row)
...
def find_device():
    for path in sorted(glob.glob("/sys/class/hidraw/hidraw*")):
        try:
            ...
            continue
        if "Preonic" in uevent and b"\x06\x60\xff" in rdesc:
            return "/dev/" + os.path.basename(path)
    sys.exit("Preonic raw-HID interface not found — is the board plugged in?")
